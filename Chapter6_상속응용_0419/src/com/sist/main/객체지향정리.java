...
/*	
 		...
 		1. 구성요소
 			1) 변수
 				...
 				= 정적변수 : 메모리 공간 한개를 사용한다 (공유변수, 클래스변수)
 					-static	=> 컴파일시 자동으로 생성
 			...
 			2) 메소드 : 명령문의 집합 => 단락(구조적인 프로그램)
 				...
 				1. 종류
 					...
 					추상 메소드 : 설계 => 모든 개발자가 같은 메소드를 제어
 								  => 표준화 작업
 								  => 여러개의 클래스르 묶어서 한개의 이름으로 제어
 							
 					종단 메소드 : final void aaa() => 변경할 수 없는 메소드 (확장없음)
 			3) 생성자 : 
 				= 객체 생성시 호출되는 메소드
 				= 멤버변수의 초기화
 				= 클래스명과 동일
 				= 리턴형이 없다
 				= 생성자는 여러개 존재할 수 없다 (오버로딩)
 		...
 		3. 접근지정어
 			...
 			----------------------------------
 			protected : 같은 패키지안에서만 접근 가능, 상속이 있는 경우 다른 패키지에 접근이 가ㅡㅇ
 			----------------------------------
