 			 ...
 			 변수
 			 	1) 멤버변수 (인스턴스 변수 => new 메모리에 저장할 때 따로 저장되는 변수)
 			 		형식) [접근지정어] 데이터형 변수명 = 값 (디폴트값)
 			 			...
 			 			=> int a;
 			 			   a=10; => 오류. 클래스에서는 구현x => 변수의 초기화 (생성자)
 			 			=> 생성시점 : new를 이용해서 메모리 공간 확보
 			 			   소멸시점 : 객체=null이거나 사용하지 않는 경우
 			 			   			------------- 가비지 컬렉션 (메모리 회수)
 			 			=> 가장 많이 선언되는 변수 (여러개를 저장)
 			 			=> 사용하는 방법
 			 				class A {
 			 					int age;		//0
 			 					String name;	//null
 			 		...
 			 		===========================> class는 한개만 설계 ==> 저장을 new를 이용해서 여러개
 			 			b=a; => 저장된 데이터관리 위치가 변경 될 수 있다(데이터손실)
 			 		
 			 		
 			 	2) 정적변수(클래스변수, 공유변수) => 한개의공간만 생성
 			 		=> 공유변수 : 오라클
