...
public class SeoulSystem {
	...
	private int curpage; //프로그램 종료시에도 유지
	...
	public SeoulLocationVO[] nameFind(String fd) { 
		...
		int count=0;
		for(SeoulLocationVO vo : datas) { 	//찾는게 몇갠지 모르기 때문에 배열로 갯수확인
		...
		//고정 => 가변 (컬렉션)
		SeoulLocationVO[] seoul = new SeoulLocationVO[count]; //찾는게 몇갠지 모르기 때문에 배열로 갯수확인 ******
	...
	public void process() {
		...
		while(true) {
			...
			if(menu==9) {
				...
				System.out.println(curpage + " page / "+ totalpage + " totalpage / ");
				
				SeoulLocationVO[] data = seoulList(curpage);
				for(SeoulLocationVO vo : data) {
					
					if(vo!=null) {	//마지막페이지는 3개라 배열할당부족으로 오류남. null이 아닐때까지만 출력하고 null이면 출력안하게끔
						System.out.println(vo.getNo()+"."+vo.getName()); 
					}
