...
/*
 	...
 	1. String은 "고정형" => 변경이 되면 새로운 메모리를 생성
 				----	 원본은 그대로 유지 (변하지않는 문자열 생성)
 		예) 
 			String s = "Hello Java";
 			String ss = s.substring(0,5)	//s값은 계속 유지됨(Hello Java)
 			=> s값 변경해서 저장하려면...
 			   s= s.substring(0,5)
 			=> 변경시에 리턴형이 String
 	...
 	2. String을 보완 => 가변형 StringBuffer(비동기) / StringBuilder(동기)
 		 **프로그램에서 동기(한개씩 수행) /비동기(동시에 여러개 일 수행)
 		 			--  		   --- 
 		 네트워크 : 클라이언트(요청) / 서버(응답)
 		 		 => 오라클 (자바_client(요청=>SQL) / 오라클_server)
...
import java.io.*;
import java.net.*;
// => 클래스 => CheckedException (예외처리 해줘야함)
