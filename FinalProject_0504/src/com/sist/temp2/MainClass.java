...
/*
 	...
 	1) 변수 (25 page)
 	   ...
 	   -------------------------------------------------
 	   지역변수 (25page) ==> 메소드가 호출될때			메소드안에서만 사용가능
 	   변수 : 한개의 데이터를 저장하는 공간
 	   우선순위 : 변수명이 동일할때 우선순위
 	   => 지역변수 , 매개변수
 	      -----반드시 초기화후에 사용
...
----------------------------------------------------------------------------------------	   
	...
	변수 생성 방법 (식별자) => 같은 메소드 상에서는 같은 변수가 존재하지 않는다
	= 식별자
...
---------------------------------------------------------------------------------------- (25~37 page)
	...
	이항 연산자
		...
		=> 논리연산자: &&(직렬) => 두개가 true일때 true
					...
					*** 효율적인 연산
					&& => 앞에 조건 false => 뒤에 조건은 수행 안함
					----포함
					|| => 앞에 조건 true => 뒤에 조건은 수행 안함
					----미포함
		=> 대입연산자: 결과값 대입
					= , += , -=
					int a=10;
