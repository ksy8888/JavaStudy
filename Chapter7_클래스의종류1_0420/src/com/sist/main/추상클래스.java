		...
		*** 기본 목적 : 여러개의 클래스를 모아서 한개의 이름으로 제어
		...
		=========
		 ...
		 예)
		 	...
		 	댓글 게시판 : 목록출력 , 찾기 , 글쓰기, 내용보기 , 수정하기 , 삭제하기, 댓글
		 	후기 게시판 : 목록출력 , 찾기 , 글쓰기, 내용보기 , 수정하기 , 삭제하기, 업로드
			...
			*** 권장 사항 ==> 한개이상의 추상메소드를 포함한다
								   ------- 구현이 안된 메소드
				abstract void find(); >> abstract 구현 안된 메소드
				-----------------------------------------------
				Board content() {} 
				void write() {}				>> 구현 된 메소드
				-----------------------------------------------
			...
			3) 상속을 내려서 구현된 클래스를 이용해서 메모리 할당
				...
				class B extends A
				------------------- 구현이 안된 메소드를 구현한다
				=> A a = new B(); 묵시적 형변환 : 작은 클래스를 큰 클래스에 넣는 것
					== double d = 10; 자동 형변환
				=> A a = new B();
				   B b = (B)a; =====> 명시적 형변환
...
/*
 	1. 일반 클래스 => new (O)
 	2. 추상 클래스(abtract) => 구현 후 사용
 	3. 인터페이스 => 추상클래스와 동일 (O)
 	4. 종단클래스(final) => 상속해서 확장이 불가능
...
abstract class A
{
	public abstract void aaa();	//abstract로 선언하면 상속받은 클래스에서 무조건 메소드구현 해야함
	public  void bbb() { }
	public  void ccc() { }
	public  void ddd() { }
