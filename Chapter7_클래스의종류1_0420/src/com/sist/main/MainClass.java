 		...
 		클래스의 종류
 		  ...
 		  2) 인터페이스 : 추상 클래스의 단점을 보완
 		  				...
 		  				public interface Inteface명
 		  				{
 		  					...
 		  					메소드
 		  						...
 		  						문제점)
 		  							interface A
 		  							{
 		  								private int a = 10; ==> 오류. (private못씀 public써야함)
 		  								(public abstract) void display();
 		  								---------------
 		  								생략되어 있음. B에서 접근지정어 축소해서 오류남
 		  							}
 		  			...
 		  			*** 추상클래스 , 인터페이스는 상속을 받는 경우
 		  				반드시 선언만 된 메소드를 구현해서 사용한다 = 선언된 모든 메소드를 구현해줘야함
 		  				
 		  			*** 추상 클래스 : 입출력, 네트워크, 데이터연결
 		  			*** 인터페이스 : 윈도우 이벤트
 		  						  버튼 클릭, 마우스 클릭, 키보드...
 		  3) 내부클래스 (네트워크 , 빅데이터)
 		  	멤버클래스 : 스레드 => 데이터를 공유할 목적
 		  	class A (Server)
 		  	{
 		  		A,B,C,D,E => 변수나 메소드
 		  		class B (통신)
